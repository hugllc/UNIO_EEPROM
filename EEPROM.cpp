...
# define ESP_OK SPI_FLASH_RESULT_OK
...
bool EEPROMClass::commit() {
  ...
  if(spi_flash_erase_sector(_sector) == ESP_OK) {
    if(spi_flash_write(_sector * SPI_FLASH_SEC_SIZE, reinterpret_cast<uint32_t*>(_data), _size) == ESP_OK) {
      _dirty = false;
      ret = true;
    }
  }
...
#elif defined(ARDUINO_ARCH_ESP32)
EEPROMClass EEPROM((0x1FD000 / SPI_FLASH_SEC_SIZE));
#endif
